...
export default class AdminConfigConsole extends LightningElement {
    ...
    async handleSaveToggles() {
        this.isSaving = true;
        try {
            // Save each toggle individually since the Apex method expects individual parameters
            for (const toggle of this.featureToggles) {
                await updateFeatureToggle({ 
                    developerName: toggle.id,
    ...
    async handleSaveAllConfigs() {
        this.isSaving = true;
        try {
            // Save each config individually since the Apex method expects individual parameters
            for (const config of this.appConfigs) {
                await updateAppConfig({
                    configId: config.id,
                    value: String(config.value)
                });
