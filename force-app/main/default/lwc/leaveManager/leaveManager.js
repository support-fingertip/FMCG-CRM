...
export default class LeaveManager extends LightningElement {
    ...
    async handleSubmitLeave() {
        ...
        // Refresh balance from server before validating to avoid stale cached data
        ...
        if (cfg) {
            ...
            if (days > available) {
                this.showToast('Error',
                    'Insufficient ' + this.leaveForm.Leave_Type__c + ' balance. Available: ' + available + ' days, Requested: ' + days + ' days.',
                    'error'
                );
                return;
            }
