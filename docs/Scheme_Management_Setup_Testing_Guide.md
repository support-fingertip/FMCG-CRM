...
## 9. Recent Changes & Enhancements

### 9.1 Product Image URL in Scheme Manager

Product search dropdowns (Buy products, Free products, Get products) now display product images when available.

- **Field:** `Product_Extension__c.Product_Image_URL__c` (URL type)
- **Where it shows:** All three product search dropdowns in the scheme wizard (Step 3: Products)
- **Behavior:** If the product has an image URL, a 32x32px thumbnail is shown. If not, a placeholder icon is displayed.
- **Apex change:** `SchemeManagerController.searchProducts()` now queries `Product_Image_URL__c`

### 9.2 Territory Lookup in Geo & Customer Mapping

The Territory field (Lookup to `Territory_Master__c`) is now exposed in the scheme wizard Step 5.

- **Search:** Type territory name or code to search (minimum 2 characters)
- **Auto-populate:** Selecting a territory auto-fills:
  - Zone / Region → from `Territory_Master__c.Region__r.Name`
  - Area / State → from `Territory_Master__c.State__c`
  - District / City → from `Territory_Master__c.City__c`
- **Manual override:** Zone, Area, District, Sub Zone fields remain editable after auto-population
- **Detail view:** Territory column is now shown in the mapping table
- **Apex change:** `SchemeManagerController.searchTerritories()` — new method for territory search
- **Apex change:** `SchemeManagerController.getScheme()` — now queries `Territory__r` fields in mappings
- **Apex change:** `SchemeViewController.getSchemeDetails()` — now includes `Territory__r.Name` in mappings

### 9.3 Customer Types Aligned with Account Record Types

**Old values (removed):** D2R, Wholesale, Modern Trade, Institutional

**New values (aligned with Account RecordTypes):**

| Picklist Value | Account RecordType DeveloperName |
|----------------|----------------------------------|
| Retailer | Retailer |
...
- **Metadata updated:** `Scheme_Mapping__c.Customer_Type__c` picklist values
- **LWC updated:** `CUSTOMER_TYPE_OPTIONS` constant in `schemeManager.js`
- **Script updated:** `scripts/11_schemes.apex` — mapping data uses new values

### 9.4 Customer Type Filtering in Scheme Engine

Previously, `Customer_Type__c` on `Scheme_Mapping__c` was stored but **never used for filtering**. Now the scheme engine actively filters by customer type.

**How it works:**

1. When evaluating schemes for an outlet, the engine reads the Account's `RecordType.DeveloperName`
2. It maps the RecordType to the corresponding Customer_Type value using `getCustomerTypeFromRecordType()`
3. `getMappedSchemeIds()` now filters: `Customer_Type__c = :customerType OR Customer_Type__c = null`

**Mapping logic:**
```
Account RecordType    →  Customer_Type__c Match
─────────────────────    ─────────────────────
Retailer              →  Retailer
Distributor           →  Distributor
...
**Behavior:**
- A mapping with `Customer_Type__c = 'Retailer'` will only match Retailer accounts
- A mapping with `Customer_Type__c = null` (blank) will match ALL customer types
- If no mappings exist at all, the scheme applies globally (unchanged behavior)

**Apex changes:** `SPM_SchemeEngine_Service.cls`
- `getAccountDetails()` — now queries `RecordType.DeveloperName`
- `getCustomerTypeFromRecordType()` — new helper method
- `getMappedSchemeIds()` — now accepts `customerType` parameter and filters by it

### 9.5 Scheme Applicability Rules (Summary)

A scheme is visible to an outlet when ALL of these conditions are true:

| Check | Condition |
|-------|-----------|
| **Status** | `Is_Active__c = true` (Status = Active AND within date range) |
| **Date** | `Start_Date__c <= today AND End_Date__c >= today` |
| **Channel** | `scheme.Channel__c = outlet.Channel__c OR scheme.Channel__c = null` |
| **Outlet Type** | `scheme.Outlet_Type__c = outlet.Outlet_Type__c OR scheme.Outlet_Type__c = null` |
| **Territory** | No mappings exist (global) OR a mapping matches (Account + Customer Type) |
| **Customer Type** | No mappings exist OR mapping.Customer_Type__c matches RecordType OR is null |
| **Product** | Product is in scheme's product list, or scheme is invoice-level |
| **Budget** | `Budget_Used__c < Budget_Amount__c` (if budget is set) |

### 9.6 Dummy Data (scripts/11_schemes.apex)

The seed script creates 8 schemes covering various category/type combinations:

| Code | Name | Category | Type | Status |
|------|------|----------|------|--------|
| SCH-2026-001 | Chips Buy 3 Get 1 Free | Free Products | Same Product (QTY) | Active |
| SCH-2026-002 | Biscuits MOV Free Product | Free Products | Same Product (VAL) | Active |
| SCH-2026-003 | Snacks & Noodles 3% Off | Discount in % | Assorted Product (QTY) | Active |
| SCH-2026-004 | Detergent Invoice Qty Discount | Discount in Value | Invoice Qty Based | Active |
| SCH-2026-005 | Invoice Value Reward Points | Reward Points | Invoice Val Based | Active |
| SCH-2026-006 | Juice Qty Reward Points | Reward Points | Same Product (QTY) | Active |
| SCH-2026-007 | Personal Care Volume Discount | Discount in % | Same Product (QTY) | Active |
| SCH-2026-008 | Modern Trade Combo Offer | Discount in Value | Assorted Product (VAL) | Draft |

**To run:** `sf apex run -f scripts/11_schemes.apex`

**Prerequisites:** Run `scripts/04_products.apex` and `scripts/02_territory_master.apex` first.
